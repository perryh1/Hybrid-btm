...
def get_live_data():
    ...
    except: return pd.Series(np.random.uniform(15, 45, 744)), 800.0, 20.0

price_hist, ghi, ws = get_live_data()
...
with c2:
    ...
    m_cost = st.slider("Miner Cost ($/TH)", 1.0, 50.0, 15.0) # Restored
with c3:
...
stats_2025 = TREND_DATA["Negative (<$0)"]["2025"] + TREND_DATA["$0 - $0.02"]["2025"]
ann_alpha = (stats_2025 * 8760 * miner_mw * (breakeven - 12)) + (0.005 * 8760 * batt_mw * 1200)
...
with col_r:
    ...
    # 1Y Trend Logic
    m_y = (TREND_DATA["Negative (<$0)"]["2025"] + TREND_DATA["$0 - $0.02"]["2025"]) * 8760 * miner_mw * (breakeven - 12)
