...
def check_password():
    ...
    st.markdown("""
    Most renewable projects operate as passive infrastructure. This application is the **economic brain** transforming the Midland site into a high-frequency trading desk.
    """)

    st.markdown("---")
    st.header("🍯 The 'Secret Sauce': The 123 on Hybrid Alpha")
    st.info("**Core Value:** The system creates a pivot that treats Bitcoin miners and batteries as a 'virtual load' reacting to market conditions in milliseconds.")
    
    col1, col2 = st.columns(2)
    with col1:
        st.markdown("**🚀 Battle-Tested at Helios**\nIntegrates direct operational learnings from Helios deployments.")
        st.markdown("**📊 5-Year Training**\nTrained on 5-min grid data to recognize market 'fingerprints'.")
        st.markdown("**❄️ Uri-Proof Backtesting**\nProven to protect assets during extreme grid collapse events.")
    with col2:
        st.markdown("**🧠 AI Battery Logic**\nMaintains 'dry powder' by predicting spikes via ambient temp and load.")
        st.markdown("**⚡ Real-Time Breakeven**\nFloor recalibrates instantly as Hashprice or Efficiency shifts.")
        st.markdown("**⏱️ Interconnect Stop-Gap**\nPowers miners today, turning a 'waiting game' into a 'revenue game'.")
...
def get_site_data():
    try:
        ...
        start = end - pd.Timedelta(days=180) # Pulling 180 days for Tab 2
...
with tab1:
    ...
    # Historical Boxes (V1 Logic)
    st.markdown("---")
    def run_stats(p_series, m_mw, b_mw, gen_mw):
        ma, ba, base = 0, 0, 0
        for p in p_series:
            base += (gen_mw * p)
            if p < breakeven:
    ...
    t_gen_avg = total_gen if total_gen > 0 else (solar_cap + wind_cap) * 0.3
    with h1: 
        m, b, g = run_stats(price_hist.tail(24), miner_mw, batt_mw, t_gen_avg)
        st.metric("Last 24h Rev", f"${(m+b+g):,.0f}", f"${m+b:,.0f} Alpha")
    with h2:
        m, b, g = run_stats(price_hist.tail(168), miner_mw, batt_mw, t_gen_avg)
        st.metric("Last 7d Rev", f"${(m+b+g):,.0f}", f"${m+b:,.0f} Alpha")
    with h3:
        m, b, g = run_stats(price_hist.tail(720), miner_mw, batt_mw, t_gen_avg)
        st.metric("Last 30d Rev", f"${(m+b+g):,.0f}", f"${m+b:,.0f} Alpha")
    with h4:
        st.metric("Last 6mo (Static)", f"${(BASE_REVENUE['6m_grid_solar'] + (BASE_REVENUE['6m_mining_per_mw']*miner_mw*0.4)):,.0f}")
    with h5:
        st.metric("Last 1yr (Static)", f"${(BASE_REVENUE['1y_grid_solar'] + ann_alpha):,.0f}")

# --- TAB 2: LIVE SPIKE ANALYSIS ---
with tab2:
    st.subheader("180-Day Live Spike Analysis")
    st.markdown("Calculating performance based on **raw market segments** and actual grid scarcity events.")
    
    ma180, ba180, g180 = 0, 0, 0
    scarcity_hrs = 0
    for p in price_hist:
        g180 += (t_gen_avg * p)
        if p > 500: scarcity_hrs += 1
        if p < breakeven:
    ...
    s1.metric("Total 180-Day Revenue", f"${(ma180+ba180+g180):,.0f}")
    s2.metric("Hybrid Alpha (Real)", f"${(ma180+ba180):,.0f}")
    s3.metric("Scarcity Events", f"{scarcity_hrs} Hours", help="Hours > $500/MWh")
    
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=price_hist.index, y=price_hist.values, name="Price", line=dict(color='#00FFCC')))
    fig.add_hline(y=breakeven, line_dash="dash", line_color="red", annotation_text="Breakeven")
